...
| P1-45 | [ ] | GitAuthorizationFilter도 SvnAuthorizationFilter와 동일한 권한 축소 보유 | `config/git/GitAuthorizationFilter.kt` | `config/git/GitAuthorizationFilter.kt` | P1-23에서 식별 — `GitAuthorizationFilter`가 `SvnAuthorizationFilter`와 완전히 동일한 구조(`requiresAuth = projectScope==PRIVATE \|\| ...`)를 갖고 있어, PROTECTED 프로젝트가 PUBLIC과 동일하게 인증 없이 clone 가능하고 게스트 계정 차단도 없다. 다만 이 백로그 항목은 감사 리포트가 SVN(`SvnApp.java`)만 명시적으로 지목해 이번 패스에서는 SVN 필터만 고쳤다. yona의 git 스마트 HTTP 경로(`GitBucketApp` 등)가 실제로 동일한 `AccessControl.isAllowed` 규칙을 쓰는지 원본 확인 후 동일한 방식(PROTECTED→PRIVATE와 동일 취급 + `isGuestUser` 거부)으로 고치면 됨 |
