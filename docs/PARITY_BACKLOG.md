...
| P1-46 | [ ] | git push(NEW_COMMIT) 알림 메일은 여전히 발송되지 않음(수신자 미계산) | `NotificationEvent.java:604-680`(push 부분, 메일 경로) | `domain/event/GitPostReceiveEventListener.kt` | P1-25에서 식별 — 웹훅은 이번에 고쳤지만, `processCommitsNotification`이 만드는 `NotificationEvent`는 `receivers`를 계산하지 않고(`WatchService` 미사용) `eventPublisher.publishEvent()`도 호출하지 않아 `NotificationMailEventListener`(P0-01)가 이 이벤트를 구독할 기회조차 없다. 즉 push 커밋에 대한 메일 알림은 여전히 발송 안 됨. `WatchService.findActualWatchers(baseWatchers=emptySet(), resourceType=PROJECT, resourceId=project.id, projectId=project.id, eventType=NEW_COMMIT)`로 프로젝트 감시자를 계산해 `receivers`에 채운 뒤 publish하면 됨 |
