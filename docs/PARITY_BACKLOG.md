...
| P1-47 | [ ] | 수신메일 HTML 서식 보존 및 cid 인라인 이미지 치환 미지원 | `CreationViaEmail.java postprocessForHTML/replaceCidWithAttachments` | `domain/mail/ImapMailboxPoller.kt`, `domain/mail/IncomingMailProcessingService.kt` | P1-29에서 범위 분리 — 첨부파일 저장은 구현했지만, (1) 본문이 text/html뿐인 메일은 여전히 jsoup으로 태그를 벗겨 순수 텍스트로만 저장(서식·링크 손실), (2) 본문 HTML 안의 `cid:xxx` 인라인 이미지 참조를 실제 저장된 첨부파일 URL로 치환하는 로직이 없어 인라인 이미지가 깨진 채로 남는다. 전자는 yuna Issue/Posting 본문이 Markdown 기준이라 raw HTML을 그대로 넣었을 때 렌더링이 실제로 안전한지 확인이 먼저 필요하고, 후자는 첨부파일 저장(P1-29) 이후 생성된 Attachment의 URL을 알아야 치환 가능해 순서 의존성이 있음 |
