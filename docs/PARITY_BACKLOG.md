...
| P1-27 | [ ] | 알림 메일이 이벤트별 즉시 발송이며 다이제스트 병합/언어별 그룹핑이 없음 | `NotificationMail.java:99-188`(`mergeEvents`, 언어별 그룹핑) | `domain/notification/NotificationMailEventListener.kt` | P0-01에서 범위 분리 — yona는 `bymail.interval` 주기로 관련 이벤트를 병합해 한 통으로 보내지만, yuna는 이벤트 발생 즉시 개별 발송(스팸성 다건 메일 가능성). 사용자가 많은 프로젝트에서 체감 UX 저하. **2026-08-20 재검토 결과 이번 세션에서 착수하지 않기로 결정**: (1) yona `mergeEvents()`는 상태변경↔댓글 이벤트를 수신자 집합 차이에 따라 최대 3갈래로 쪼개 병합하는 정교한 알고리즘이라 충실한 이식 자체가 큰 작업, (2) 현재 즉시발송 아키텍처(P0-01, 6 tests로 검증됨)를 지연-배치-병합 구조로 통째로 바꿔야 해서 기존 테스트 계약이 전부 깨짐, (3) "언어별 그룹핑"은 yuna가 이미 수신자별 개별 발송 구조를 택해 원본의 전제(여러 수신자를 한 메일로 묶어 언어별로 나눔)와 안 맞음. 정확성/보안이 아닌 UX(스팸성 다건 메일) 이슈라 이번 시간 예산에서는 후순위로 미루고 다른 항목을 우선 처리 |
| P1-28 | [x] | 알림 메일에 IMAP 답장용 Reply-To 헤더 없음 | `NotificationMail.java:582 getReplyTo()` | `domain/notification/NotificationMailEventListener.kt` | **완료(범위 조정, 아래 참고)** |
...
- **2026-08-20 — P1-22**: 사용자가 특정 프로젝트의 특정 이벤트 타입 알림을 끌 수 있는 `UserProjectNotification`(뮤트 토글) 엔티티·리포지토리·설정 API(`WatchController`)는 이미 yuna에 있었지만, 실제 알림 수신자를 계산하는 어느 코드에서도 이 설정을 조회하지 않아 뮤트를 꺼도 계속 알림이 오던 문제 수정(yona `NotificationEvent.filterReceivers()`의 `UserProjectNotification.isEnabledNotiType()` 대응).
  - yona `filterReceivers()`를 그대로 옮기는 대신, 이 저장소의 모든 알림 발행 경로가 공통으로 거치는 `WatchService.findActualWatchers()`에 통합했다 — `IssueServiceImpl`/`CommentServiceImpl`/`PostingServiceImpl` 개별 수정 없이 한 곳에서 일관되게 적용되도록 하기 위함(단, 호출부는 실제 `eventType`을 넘겨줘야 하므로 7개 호출부 모두에 `eventType = notificationEvent.eventType` 인자를 추가했다).
...
- **2026-08-20 — P1-25**: git push 시 커밋 알림용 `NotificationEvent`는 저장되고 있었지만 아무도 구독하지 않는(=`eventPublisher.publishEvent()`를 호출하지 않는) 채로 방치돼 있어, push(NEW_COMMIT)에 대해서는 웹훅이 한 번도 발송되지 않던 문제 수정(yona `Webhook.sendRequestToPayloadUrl(List<RevCommit>, List<String>, User)` 대응).
  ...
  - 테스트: `GitPostReceiveEventListenerSpec.kt` +2(신규 — 커밋이 있으면 알림 저장+웹훅 발송 확인, 커밋이 없으면 아무 것도 안 함) — `RevCommit`은 실제 값이 필요해(protected 생성자) `InMemoryRepository`(JGit DFS)로 진짜 커밋을 만들어 테스트. 기존 4건 포함 총 6 tests. `WebhookServiceSpec`/`WebhookNotificationEventListenerSpec`/`YonaApplicationTests` 재실행으로 회귀 없음 확인.
