...
class IssueLabelController(
    ...
    fun createLabel(
        ...
        if (!accessControl.isProjectResourceCreatable(user, project, ResourceType.ISSUE_LABEL)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun createCategory(
        ...
        val user = getLoginUser(authentication) ?: return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build()
        if (!accessControl.isProjectResourceCreatable(user, project, ResourceType.ISSUE_LABEL_CATEGORY)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun updateLabel(
        ...
        val user = getLoginUser(authentication) ?: return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build()
        if (!accessControl.isAllowed(user, project, ResourceType.ISSUE_LABEL, Operation.UPDATE)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun updateCategory(
        ...
        val user = getLoginUser(authentication) ?: return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build()
        if (!accessControl.isAllowed(user, project, ResourceType.ISSUE_LABEL_CATEGORY, Operation.UPDATE)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun copyLabels(
        ...
        val user = getLoginUser(authentication) ?: return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build()
        if (!accessControl.isProjectResourceCreatable(user, toProject, ResourceType.ISSUE_LABEL)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun deleteLabel(
        ...
        val user = getLoginUser(authentication) ?: return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build()
        if (!accessControl.isAllowed(user, project, ResourceType.ISSUE_LABEL, Operation.DELETE)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun deleteCategory(
        ...
        val user = getLoginUser(authentication) ?: return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build()
        if (!accessControl.isAllowed(user, project, ResourceType.ISSUE_LABEL_CATEGORY, Operation.DELETE)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
